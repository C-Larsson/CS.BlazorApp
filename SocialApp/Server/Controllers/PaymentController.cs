...
namespace SocialApp.Server.Controllers
{
    ...
    public class PaymentController : ControllerBase
    {
        ...
        [HttpPost("confirm"), Authorize]
            public async Task<ActionResult<ServiceResponse<List<Reservation>>>> ConfirmPayment(PaymentRequest request)
        {
            var response = await _paymentService.ConfirmPayment(request.Reservations);
            if (!response.Success)
                return BadRequest(response.Message);

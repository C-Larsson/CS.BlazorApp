...
namespace SocialApp.Client.Services.AuthService
{
    public class AuthService : IAuthService
    {
        ...
        {
            var result = await _http.PostAsJsonAsync("/api/auth/change-password/", userChangePassword);
            return await result.Content.ReadFromJsonAsync<ServiceResponse<bool>>();
        }

        public async Task<string> GetUserName()
        {
            var username = (await _authStateProvider.GetAuthenticationStateAsync()).User.Identity.Name;
            return username;
        }
    }
}
